...
class Agent:
    ...
    def choose_action(self, observation):
        ...
        noise = T.tensor(np.random.normal(scale=self.noise, size=(self.n_actions,)),
                         dtype=T.float).to(self.actor.device)
        mu_prime = mu + noise

        min_act = T.tensor(self.min_action, dtype=T.float).to(self.actor.device)
        max_act = T.tensor(self.max_action, dtype=T.float).to(self.actor.device)
    ...
    def learn(self):
        ...
        noise = T.tensor(np.random.normal(scale=0.2, size=target_actions.shape),
                         dtype=T.float).to(self.critic_1.device)
        noise = T.clamp(noise, -0.5, 0.5)

        min_act = T.tensor(self.min_action, dtype=T.float).to(self.critic_1.device)
        max_act = T.tensor(self.max_action, dtype=T.float).to(self.critic_1.device)

        target_actions = T.clamp(target_actions + noise, min_act, max_act)

        q1_ = self.target_critic_1.forward(state_, target_actions).squeeze(-1)
        q2_ = self.target_critic_2.forward(state_, target_actions).squeeze(-1)

        critic_value_ = T.min(q1_, q2_)

        target = reward + (1.0 - done.float()) * self.gamma * critic_value_
